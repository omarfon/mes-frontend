...
import { PlantsService, Plant, CreatePlantDto } from './plants.service';
...
export class PlantsComponent implements OnInit {
  form: CreatePlantDto = {
    code: '', name: '', country: '', city: '', timezone: 'America/Bogota', active: true,
  };

  items: Plant[] = [];
  editingId: string | null = null;
  currentItem: Plant | null = null;
  formPanelOpen = false;
  viewOnly = false;
  q = '';
  loading = false;
  ...
  readonly timezones = [
    'America/Bogota', 'America/Lima', 'America/Guayaquil', 'America/Santiago',
    'America/Buenos_Aires', 'America/Sao_Paulo', 'America/Mexico_City', 'America/Caracas',
    'Europe/Madrid', 'UTC',
  ];

  constructor(private svc: PlantsService, private cdr: ChangeDetectorRef, private authSvc: AuthService, private confirmSvc: ConfirmService, private toast: ToastService) {}
  ...
  get filtered() {
    ...
    return this.items.filter(x =>
      [x.code, x.name, x.country, x.city].some(v => v.toLowerCase().includes(t))
    );
  }
  ...
  edit(it: Plant) {
    ...
    this.currentItem = it;
    this.form = { code: it.code, name: it.name, country: it.country, city: it.city, timezone: it.timezone, active: it.active };
    this.viewOnly = false;
    this.formPanelOpen = true;
  }
  ...
  remove(id: string) {
    this.confirmSvc.open({ title: 'Eliminar planta', message: '¿Estás seguro? Esta acción no se puede deshacer.' })
      .subscribe(ok => {
        if (!ok) return;
        this.svc.delete(id).subscribe({
          next: () => { this.toast.show('Planta eliminada'); this.load(); if (this.editingId === id) this.cancelEdit(); },
  ...
  resetForm() {
    this.form = { code: '', name: '', country: '', city: '', timezone: 'America/Bogota', active: true };
  }
